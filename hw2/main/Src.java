...
class While extends Stmt {
  ...
  Code compile(Program prog, Code next, Code breakNext, Code continueNext) {
    ...
    head.set(test.compileTo(prog, tmp,
             new Cond(tmp,
                      prog.block(body.compile(prog, loop, got, loop)),
                      prog.block(next))));
    return loop;
  }
}
...
class If extends Stmt {
  ...
  Code compile(Program prog, Code next, Code breakNext, Code continueNext) {
    ...
    Goto  got = new Goto(n);
    Block t   = prog.block(ifTrue.compile(prog, got, breakNext, continueNext));
    Block f   = (ifFalse==null)
                 ? n : prog.block(ifFalse.compile(prog, got, breakNext, continueNext));
