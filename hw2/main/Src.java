...
abstract class Expr {
  ...
  abstract Code compileTo(Program prog, Tmp reg, Code next);
}
...
class Var extends Expr {
  ...
  Code compileTo(Program prog, Tmp reg, Code next) {
...
class Plus extends ArithBinExpr {
  ...
  Code compileTo(Program prog, Tmp reg, Code next) {
    Tmp tmp = new Tmp();
    return l.compileTo(prog, tmp,
           r.compileTo(prog, reg,
...
class LAnd extends LogicBinExpr {
  ...
  Code compileTo(Program prog, Tmp reg, Code next) {
    ...
    return l.compileTo(prog, reg,
                       new Cond(reg,
                                prog.block(r.compileTo(prog, reg,
                       new Cond(reg,
...
abstract class Stmt {
  ...
  Code compile(Program prog, Code next){
    Code code = compile(prog, next, null, null);
    return code;
  }

  abstract Code compile(Program prog, Code next, Code breakNext, Code continueNext);
}
...
class Seq extends Stmt {
  ...
  Code compile(Program prog, Code next, Code breakNext, Code continueNext) {
    return l.compile(prog, r.compile(prog, next, breakNext, continueNext), breakNext, continueNext);
  }
}
...
class Assign extends Stmt {
  ...
  Code compile(Program prog, Code next, Code code, Code code1) {
    Tmp tmp = new Tmp();
    return rhs.compileTo(prog, tmp, new Store(te.getLoc(), tmp, next));
  }
}
...
class While extends Stmt {
  ...
  Code compile(Program prog, Code next, Code code, Code code1) {
    ...
    Tmp   tmp  = new Tmp();
    head.set(test.compileTo(prog, tmp,
             new Cond(tmp,
                      prog.block(body.compile(prog, loop, null, null)),
                      prog.block(next))));
    return loop;
  }
}
...
class If extends Stmt {
  ...
  Code compile(Program prog, Code next, Code code, Code code1) {
    ...
    Goto  got = new Goto(n);
    Block t   = prog.block(ifTrue.compile(prog, got, null, null));
    Block f   = (ifFalse==null)
                 ? n : prog.block(ifFalse.compile(prog, got, null, null));
    return test.compileTo(prog, tmp, new Cond(tmp, t, f));
  }
}
...
class Print extends Stmt {
  ...
  Code compile(Program prog, Code next, Code code, Code code1) {
    Tmp tmp = new Tmp();
    return exp.compileTo(prog, tmp, new PCode(tmp, next));
  }
}
...
class Return extends Stmt {
  ...
  Code compile(Program prog, Code next, Code code, Code code1) {
    Tmp tmp = new Tmp();
    return exp.compileTo(prog, tmp, new Ret(tmp));
  }
}
...
class InitVarIntro extends VarIntro {
  ...
  Code compile(Code next) {
    Tmp tmp = new Tmp();
    return expr.compileTo(null, tmp,
           new Store(te.getLoc(), tmp,
           next));
  }
}
...
class DoWhile extends Stmt {
  ...
  Code compile(Program prog, Code next, Code code, Code code1) {
    ...
    head.set(body.compile(prog,
                          test.compileTo(prog, tmp,
                          new Cond(tmp,
                                   prog.block(body.compile(prog, loop, breakNext, continueNext)),
                                   prog.block(next))), breakNext, continueNext));
    return loop;
  }
}
