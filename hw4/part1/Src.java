...
class Bool extends Expr {
  ...
  // q1-c Add compileCond implementation
  // if compileCond is used, bool is compiled differently
  /*
  Code compileCond(final llvm.Function fn,
                   final llvm.Block t,
                   final llvm.Block f) {
...
class LT extends RelBinExpr {
  ...
  // q1-c Add compileCond implementation
  // elavluate and pass result to next 
 /* Code compileCond(final llvm.Function fn,
                   final llvm.Block t,
                   final llvm.Block f) {
  //Goes to Bool somehow??
    new llvm.Cond(v, t, f);
  }
  */
  Code compileCond(final llvm.Function fn,
                   final llvm.Block t,
                   final llvm.Block f) {
    return new llvm.Goto(t); // not reached
    l.compileCond(fn, 
  }
}
...
class LAnd extends LogicBinExpr {
  ...
  // q1-c 'l' and 'r' should have compileCond methods so should 
  // use those instead of 'compile'
    ...
    return l.compile(fn, lv -> {
        ...
        Code  evalRight   = r.compile(fn, rv -> {
            ...
            llvm.Reg rg     = fn.reg(llvm.Type.i1);
                    //create 'Op' with value in reg 'rg'
  ...
  Code compileCond(final llvm.Function fn,
                   final llvm.Block t,
                   final llvm.Block f) {
    //evaluate based on results of both expr's which should be returning their val 
    // when compiled... 
    //cond on first/'l' to goto either second/'r' or next
    return l.compileCond// pass new stuff added to t and f 
    final Block second = fn.block(r.compileCond(fn, t, f));
    final Block first  = fn.block(l.compileCond(fn, second, f));
    return new llvm.Goto(first);
  }
}
...
class While extends Stmt {
  ...
  // q1-c make 'test' call 'compileCond' once it is implemented
  /*
  Code compile(final llvm.Function fn, final Code andThen) {
    final Block head = fn.block();
    final Code  loop = new llvm.Goto(head);
    head.set(test.compile(fn, v ->
        // if v is true, then v = first, else v = second?
  ...
  */
  ...
  // invoke body.compile(fn, test.compileCond(
  Code compile(final llvm.Function fn, final Code andThen) {
    final Block head = fn.block();
    final Code  loop = new llvm.Goto(head);
    //final Code  next = new llvm.Goto(fn.block(andThen));
    head.set(test.compileCond(fn, fn.block(body.compile(fn, loop)), fn.block(andThen)));
    return loop;
  }

  // cond for first test; cond for second
  // wants a phi?
  // Code compileCond(final llvm.Function fn,
  //                 final llvm.Block t,
  //                 final llvm.Block f) {
    ...
    return test.compile(fn, v -> {
      if (andThen==null) {
        ...
        Block  f   = (ifFalse==null)
                   ...
                   //br i1 %v label %ifTrue, label %ifFalse
