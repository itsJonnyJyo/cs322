...
class Nth extends LValue {
  ...
  // 2-c
  // TODO: accessed index should be greater than or equal to zero
  // and less than the 'length' 
  // update compilation rules and/or runtime library support
  ...
  Code compileLoc(final llvm.Function fn, final ValCont k) {
    return arr.compile(fn, a ->
           ...
           idx.compile(fn, i -> {
             ...
             llvm.Reg  rgI = fn.reg(llvm.Type.i32);
             ...
             // 2-a generate llvm code to increment the desired index by 1, since the
             // first element corresponds to the length
             return new llvm.Op(rgI, new llvm.Add(llvm.Type.i32, i, new llvm.IntVal(1)), 
               new llvm.Op(rg, new llvm.Getelementptr(a, rgI),
                    k.with(rg)));
             
             // TODO: look into using pointer arithmetic to avoid modifications to Nth...

             //return new llvm.Op(rg, new llvm.Getelementptr(a, i -> inc i {
             //
             //}),
           }));
  }
...
class Length extends Expr {
  ...
  Code compile(final llvm.Function fn, final ValCont k) {
    return this.compileLoc(fn, loc -> {
      llvm.Reg rg2 = fn.reg(loc.getType().ptsTo());
      return new llvm.Op(rg2, new llvm.Load(loc), k.with(rg2));
    });
  }

  // 2-a
  //use compileLoc() pattern inspired by Nth and LValue
  Code compileLoc(final llvm.Function fn, final ValCont k) {
    return arr.compile(fn, a -> {
           // "a" is a value that holds the address of the array
             ...
             // address "a", save the result in register "rg", and then
             // pass "rg" as the input to the continuation "k":
             return new llvm.Op(rg, new llvm.Getelementptr(a, new llvm.IntVal(0)),
                    k.with(rg));
           });
