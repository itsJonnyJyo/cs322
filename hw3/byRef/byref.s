#I have commented all the instructions I felt were important.
#comments for instructions that I have added or changed are 
#preceded by "(CHANGE)"
...
Xmain:
    ...
    #(CHANGE) compute address in memory of said variable
        leaq    -8(%rbp), %rdx
    #put said address in 'rdi' to be passed into 'byref()'
        movq    %rdx, %rdi
    #(CHANGE) repeat above steps for second variable to be passed to 'byref()'
        movl    $23, %eax
        pushq   %rax
        leaq    -16(%rbp), %rdx
        movq    %rdx, %rsi
...
Xbyref:
    ...
    #the correct address's for x & y. Might not be necessary but
    # CALLER SAVES!!
	pushq	%rsi
	pushq	%rdi
